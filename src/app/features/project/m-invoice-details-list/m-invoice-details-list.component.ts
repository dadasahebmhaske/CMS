...
@Component({
  ...
  styleUrls: ['./m-invoice-details-list.component.css']
})
export class MInvoiceDetailsListComponent implements OnInit {
  public empInfo: any = {};
  ...
  configureGrid() {
    ...
    columnDefs = [
      ...
      { name: 'InvoiceNo', displayName: 'Invoice No', width: "*",cellClass: 'text-center', cellTooltip: true, filterCellFiltered: true },
      { name: 'InvoiceDate', displayName: 'Invoice Date', cellClass: 'text-center', width: "*", cellTooltip: true, filterCellFiltered: true },
  ...
  onLoad() {
    ...
    this.projectService.getTransactionlist(105, this.Filter).subscribe((resData: any) => {
      this.loaderbtn = true;
      if (resData.StatusCode != 0) {
        this.InvoiceData = resData.Data.Table; console.log(resData.Data);
