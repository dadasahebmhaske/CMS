...
export class WcInvoiceDetailsComponent implements OnInit, OnDestroy {
  ...
  public onSelectProject(RefTranNo) {
    ...
    this.projectService.getWorkContractDeatils(tranNo, this.project.ProjectId, RefTranNo).subscribe((resData: any) => {
      if (resData.StatusCode != 0) {
        console.log(resData);
        if (RefTranNo == '') {
          this.VendorData = resData.Data.Table1;
          this.GRNData = resData.Data.Table2;
            this.project.ContractorId='';
  ...
  public getTranData() {
    this.editflag='E';
    this.projectService.getTransDetails(110, this.project.TranNo).subscribe((resTran: any) => {
  ...
  onSelectVendor() {
    this.GRNArray = this.projectService.filterData(this.GRNData, this.project.ContractorId, 'ContractorId');
  }
  ...
  public onSubmit() {
    ...
    this.projectService.post('ManageWorkContractInvoice', ciphertext).subscribe((resData: any) => {
      this.loaderbtn = true;
      if (resData.StatusCode != 0) {
        AppComponent.SmartAlert.Success(resData.Message);
        AppComponent.Router.navigate(['/project/wc-invoice-details-list']);
      }
