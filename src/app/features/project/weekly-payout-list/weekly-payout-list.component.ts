...
@Component({
  selector: 'sa-weekly-payout-list',
  templateUrl: './weekly-payout-list.component.html',
  styleUrls: ['./weekly-payout-list.component.css']
})
export class WeeklyPayoutListComponent implements OnInit {
  public empInfo: any = {};
  ...
  configureGrid() {
    ...
    this.gridOptions.exporterExcelFilename = 'Weekly Payout list.xlsx';
  ...
  //{ name: 'IsActive', displayName: 'Active', width: "*", cellTooltip: true, filterCellFiltered: true },
    ...
    AppComponent.Router.navigate(['/project/weekly-payout']);
  }
  onDeleteFunction = ($event) => {    
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.projectService.getDeleteTransaction($event.row.TranNo, 113).subscribe((resData: any) => {
  ...
  onLoad() {
    ...
    this.projectService.getTransactionlist(113, this.Filter).subscribe((resData: any) => {
      this.loaderbtn = true;
      if (resData.StatusCode != 0) {
        this.WeeklyPayoutData = resData.Data.Table; console.log(resData.Data);
        AppComponent.SmartAlert.Success(resData.Message);
      }
      else { this.WeeklyPayoutData = [{}]; AppComponent.SmartAlert.Errmsg(resData.Message); }
    });
