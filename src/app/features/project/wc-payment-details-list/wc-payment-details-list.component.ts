...
@Component({
  selector: 'sa-wc-payment-details-list',
  templateUrl: './wc-payment-details-list.component.html',
  styleUrls: ['./wc-payment-details-list.component.css']
})
export class WcPaymentDetailsListComponent implements OnInit {  
  public empInfo: any = {};
  ...
  //{ name: 'IsActive', displayName: 'Active', width: "*", cellTooltip: true, filterCellFiltered: true },
    ...
    AppComponent.Router.navigate(['/project/wc-payment-details']);
  }
  onDeleteFunction = ($event) => {    
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.projectService.getDeleteTransaction($event.row.TranNo, 111).subscribe((resData: any) => {
  ...
  onLoad() {
    ...
    this.projectService.getTransactionlist(111, this.Filter).subscribe((resData: any) => {
      this.loaderbtn = true;
      if (resData.StatusCode != 0) {
